...
function filterLeadership(items, query, status, rank) {
  ...
  return items.filter((leader) => {
    const matchesQuery = !normalizedQuery || [leader.name, leader.position, leader.rank, leader.bio].filter(Boolean).some((field) => field.toLowerCase().includes(normalizedQuery));
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-3">
            <Badge variant={leader.status === 'Current' ? 'success' : 'warning'}>{leader.status === 'Current' ? 'Aktif' : 'Riwayat'}</Badge>
          ...
          <p className="mt-3 text-sm leading-relaxed text-slate-500">{leader.bio}</p>
        </div>
      </div>
...
export default function StructurePageClient() {
  ...
  const filteredLeadership = useMemo(() => filterLeadership(leadershipData, query, status, rank), [query, status, rank]);
  ...
  const stats = { total: leadershipData.length, active: currentLeadership.length, history: previousLeadership.length, filtered: filteredLeadership.length };
            ...
            {filteredCommander && (
              <div className="card-elevated p-6 md:p-8 lg:p-10 relative overflow-hidden">
                ...
                <div className="relative z-10 text-center">
                  ...
                  <h2 className="mt-6 text-xl md:text-2xl font-black text-slate-900">{filteredCommander.name}</h2>
                  <p className="mt-2 text-indigo-700 font-bold text-sm uppercase tracking-[0.2em]">{filteredCommander.position}</p>
                  <p className="mt-3 text-sm text-slate-500 leading-relaxed">{filteredCommander.bio}</p>
                  ...
                  <Link href={`/structure/${filteredCommander.id}`} className="mt-6 inline-flex items-center justify-center gap-2 rounded-full bg-indigo-950 px-5 py-3 text-sm font-bold text-white transition-colors hover:bg-indigo-900">
        ...
        <div className="container mx-auto section-spacing">
          <div className="card-elevated p-6 md:p-8 mb-8 md:mb-10">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                ...
                <h2 className="mt-4 text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Cari, saring, lalu fokus ke tokoh yang dibutuhkan</h2>
                <p className="mt-4 text-slate-500 leading-relaxed">Pencarian nama, jabatan, dan pangkat disusun di satu tempat supaya pengunjung tidak perlu membuka blok yang sama berulang kali.</p>
              </div>
              <div className="w-full lg:max-w-xl">
                <div className="relative">
                  <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                  <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama, jabatan, atau pangkat..." className="input-base pl-11 bg-white" aria-label="Cari pimpinan" />
                </div>
              </div>
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              {[{ label: 'Semua', key: 'all' }, { label: 'Aktif', key: 'Current' }, { label: 'Riwayat', key: 'Previous' }].map((item) => (
                <button key={item.key} type="button" onClick={() => setStatus(item.key)} className={`rounded-full border px-4 py-2 text-sm font-bold transition-all ${status === item.key ? 'border-indigo-950 bg-indigo-950 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50'}`}>
                  {item.label}
                </button>
              ))}
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              {['all', ...availableRanks].map((itemRank) => (
                <button key={itemRank} type="button" onClick={() => setRank(itemRank)} className={`rounded-xl border px-3 py-2 text-xs md:text-sm font-bold transition-all ${rank === itemRank ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50'}`}>
            ...
            {(query || status !== 'all' || rank !== 'all') && (
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm text-slate-500">Menampilkan <span className="font-bold text-slate-700">{stats.filtered}</span> pimpinan sesuai filter.</div>
                <button type="button" onClick={() => { setQuery(''); setStatus('all'); setRank('all'); }} className="text-sm font-semibold text-indigo-700 hover:text-indigo-900 transition-colors">Reset filter</button>
              </div>
            )}
          </div>
          <section className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] items-start mb-12 md:mb-16">
            <div className="card-elevated p-6 md:p-8 lg:p-10">
              ...
              <h2 className="mt-4 text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Susunan komando dalam tiga lapis yang mudah dipahami</h2>
              <p className="mt-4 text-slate-500 leading-relaxed">Bagian ini merangkum fungsi utama komando agar pembaca langsung menangkap konteks organisasi tanpa membaca pengulangan pada beberapa blok yang berbeda.</p>
              <div className="mt-8 space-y-4">
                <div className="flex gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-950 text-white"><ShieldCheck size={18} /></div>
                  <div>
                ...
                <div className="flex gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700"><Users size={18} /></div>
                  <div>
                ...
                <div className="flex gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-50 text-amber-700"><Award size={18} /></div>
                  <div>
            ...
            <div className="card-elevated p-6 md:p-8 lg:p-10">
              <div className="flex items-end justify-between gap-4 mb-6">
                <div>
                  ...
                  <h2 className="mt-4 text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Pimpinan yang sedang menjabat</h2>
                </div>
              ...
              {filteredCurrentLeadership.length > 0 ? (
                ...
                <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">Tidak ada pimpinan aktif yang cocok dengan filter saat ini.</div>
              )}
            </div>
          </section>
          <section className="card-elevated p-6 md:p-8 lg:p-10">
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between mb-6 md:mb-8">
              <div>
                ...
                <h2 className="mt-4 text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Jejak pimpinan sebelumnya</h2>
              </div>
              <p className="max-w-xl text-sm text-slate-500 leading-relaxed">Riwayat komando ditampilkan terpisah dari pimpinan aktif sehingga pengunjung bisa membedakan masa jabatan dengan cepat.</p>
            </div>
            {filteredPreviousLeadership.length > 0 ? (
              <div className="grid gap-4">
                {filteredPreviousLeadership.map((leader) => (
                  <Link key={leader.id} href={`/structure/${leader.id}`} className="group flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-5 md:p-6 transition-all hover:-translate-y-1 hover:border-indigo-300 hover:shadow-lg hover:shadow-indigo-950/5 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-start gap-4">
                      <LeaderAvatar leader={leader} size="sm" />
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <Badge variant="warning">Riwayat</Badge>
                          <span className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">{leader.tenure_start}{leader.tenure_end ? ` - ${leader.tenure_end}` : ''}</span>
                        </div>
                        <h3 className="mt-3 font-black text-slate-900 leading-tight">{leader.name}</h3>
                        <p className="mt-1 text-sm font-bold uppercase tracking-wide text-indigo-700">{leader.position}</p>
              ...
              <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-slate-500">Tidak ada riwayat komando yang cocok dengan filter saat ini.</div>
            )}
          </section>
        </div>
      </main>
